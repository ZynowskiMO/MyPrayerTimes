...
-- Colours come from the Theme module (ADR-0009); see ui/Theme.lua for the
-- Light/Dark palettes and roles.
...
function Window.create()
  ...
  local border = f:CreateTexture(nil, "BACKGROUND"); border:SetAllPoints()
  Theme.tex(border, "winBorder")
  ...
  title:SetPoint("LEFT", header, "LEFT", 10, 0); Theme.txt(title, "gold")
  f.title = title
  ...
  Icons.setUI(gt, "settings", unpack(Theme.color("gold")))
  gear:SetScript("OnEnter", function() gt:SetVertexColor(1, 0.95, 0.7) end)
  gear:SetScript("OnLeave", function() gt:SetVertexColor(unpack(Theme.color("gold"))) end)
  gear:SetScript("OnClick", function()
    local P = require("Picker"); if P and P.toggle then P.toggle() end
  end)
  f.gear, f.gearIcon = gear, gt
  ...
  Icons.setUI(mIcon, "minimize", unpack(Theme.color("gold")))
  minBtn:SetScript("OnEnter", function() mIcon:SetVertexColor(1, 0.95, 0.7) end)
  minBtn:SetScript("OnLeave", function() mIcon:SetVertexColor(unpack(Theme.color("gold"))) end)
  ...
  Theme.tex(miniIconBg, "slot"); miniIconBg:Hide()
  ...
  nextLine:SetPoint("LEFT", miniIconBg, "RIGHT", 8, 0); Theme.txt(nextLine, "text")
  nextLine:Hide()
  local miniTime = f:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
  miniTime:SetPoint("RIGHT", f, "TOPRIGHT", -14, -46); Theme.txt(miniTime, "text")
  miniTime:Hide()
...
function Window.applyMinimized()
  ...
  if f.minBtn and f.minBtn.icon then
    Icons.setUI(f.minBtn.icon, mini and "restore" or "minimize", unpack(Theme.color("gold")))
  end
...
-- Repaint the non-registry bits on a theme change: the gold gear/minimise icon
-- tints, then a full refresh (which re-applies the state-dependent row/countdown
-- colours and re-tints the prayer icons from the active palette).
function Window.applyTheme()
  local f = Window.frame
  if not f then return end
  if f.gearIcon then Icons.setUI(f.gearIcon, "settings", unpack(Theme.color("gold"))) end
  if f.minBtn and f.minBtn.icon then
    local mini = Window.db and Window.db.minimized
    Icons.setUI(f.minBtn.icon, mini and "restore" or "minimize", unpack(Theme.color("gold")))
  end
...
function Window.rowColor(key, isNext)
  if isNext then return Theme.color("nextText") end
  if key == "sunrise" then return Theme.color("sunrise") end
...
local function renderNow(now)
  ...
  for _, key in ipairs(ORDER) do
    ...
    if f.rows[key].icon then
      ...
      if key == "sunrise" and not isNext then f.rows[key].icon:SetVertexColor(unpack(Theme.color("sunrise"))) end
    end
  end
...
Theme.addHook(function() Window.applyTheme() end)
